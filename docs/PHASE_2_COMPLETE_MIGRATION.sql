-- PHASE 2: COMPLETE MIGRATION SCRIPT
...
CREATE INDEX IF NOT EXISTS idx_appointments_shop_month 
ON appointments(shop_id, date) 
WHERE status NOT IN ('cancelled', 'no_show');

-- Index for client-specific appointment queries
CREATE INDEX IF NOT EXISTS idx_appointments_shop_client_date 
...
CREATE INDEX IF NOT EXISTS idx_appointments_date_brin 
...
VALUES 
    ('20250801000001', '001_create_clients_table', ARRAY['CREATE TABLE clients...']),
    ('20250801000002', '002_create_appointments_tables', ARRAY['CREATE TABLE appointments...']),
    ('20250801000003', '003_remove_walk_in_appointments', ARRAY['ALTER TABLE appointments...']),
    ('20250804210500', '004_optimize_appointment_indexes', ARRAY['CREATE INDEX...'])
...
SELECT 
    ...
    COUNT(*) as result,
    'Should be at least 6' as expected
FROM pg_indexes
...
-- Show migration applied successfully
