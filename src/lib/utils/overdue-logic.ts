 ...
 *
 * @param garment - The garment to check
 * @param referenceDate - Optional reference date (defaults to current date). Use shop's timezone date for accurate calculations.
 */
export function isGarmentOverdue(
 ...
 *
 * @param dueDateStr - The due date string to calculate from
 * @param referenceDate - Optional reference date (defaults to current date). Use shop's timezone date for accurate calculations.
 */
 ...
 *
 * @param garment - The garment to get due date info for
 * @param referenceDate - Optional reference date (defaults to current date). Use shop's timezone date for accurate calculations.
 */
export function getEnhancedDueDateInfo(
	...
	const daysUntilDue = getDaysUntilDue(garment.due_date, referenceDate);
...
export function isOrderOverdue(
	...
	if (order.garments && order.garments.length > 0) {
		return order.garments.some((garment) =>
			isGarmentOverdue(garment, referenceDate)
		);
	}
