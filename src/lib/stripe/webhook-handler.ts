...
export async function processStripeEvent(event: Stripe.Event): Promise<void> {
	...
	switch (event.type) {
		...
		case 'charge.updated': {
			...
			// Only process if balance_transaction is now available
			// This webhook fires when async balance transaction becomes available after payment
			if (
				...
				console.log('💰 Balance transaction available for charge:', {
					charge_id: charge.id,
					payment_intent: paymentIntentId,
					fee_cents: balanceTx.fee,
					net_cents: balanceTx.net,
					from_connected_account: !!event.account,
				});
				...
				if (paymentIntentId) {
					...
					if (error) {
						console.error('Error updating payment with fee data:', error);
						throw error; // Let Stripe retry
					} else {
						console.log('✅ Successfully updated payment with fee data');
					}
				}
			}
			return;
