...
describe('testEmailTemplate', () => {
	...
	beforeEach(() => {
		...
		mockSupabase = {
			from: jest.fn((tableName: string) => {
				...
				if (tableName === 'email_signatures') {
					// Mock signature query - return no signature found
					chain.single = jest.fn().mockResolvedValue({
						data: null,
						error: { code: 'PGRST116' }, // No rows returned
					});
				} else if (tableName === 'email_logs') {
					// Mock email log insert
					chain.insert = jest
						.fn()
	...
	it('should successfully send a test email with the specified template', async () => {
		...
		// EmailRepository.getTemplate() now only uses default templates, doesn't query email_templates table
		expect(mockSupabase.from).toHaveBeenCalledWith('email_signatures');
	...
	it('should succeed with default template when no custom template exists', async () => {
		// Since EmailRepository.getTemplate() only returns default templates,
		// this test should always succeed with a default template
		const result = await testEmailTemplate('appointment_scheduled');
