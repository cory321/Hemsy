...
export default async function GarmentDetailPage({
	...
	try {
		...
		// Fetch shop hours, calendar settings, history, and balance status in parallel using cached functions
		const [
			rawShopHours,
			rawCalendarSettings,
			rawBalanceStatus,
		...
		if (rawHistoryData && rawHistoryData.success) {
			historyData = rawHistoryData.history || [];
		}
