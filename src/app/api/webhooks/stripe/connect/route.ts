...
export async function POST(request: Request) {
	try {
		...
		// Process Connect-specific events in this endpoint
		//
		// IMPORTANT: For DIRECT charges (connected account is merchant of record),
		// payment events come through THIS endpoint with event.account set.
		// For DESTINATION charges, payment events come through platform webhook.
		const connectEvents = [
			...
			'payout.failed',
			'payout.canceled',
			// Payment events for DIRECT charges (from connected account)
			'payment_intent.succeeded',
			'payment_intent.payment_failed',
			'charge.succeeded',
			'charge.updated', // CRITICAL: Needed for fee tracking on direct charges
			'charge.refunded',
			'refund.created',
		];
